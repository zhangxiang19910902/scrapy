...
def get_url():
    ...
    for year in years:
        for product in products:
            url = f'https://wits.worldbank.org/trade/comtrade/en/country/ALL/year/{year}/tradeflow/Exports/partner/WLD/product/{product}'
...
def get_content(urls):
    ...
    db2 = client.name.zx_1031_worldbank世界_Quantity
    num = 0
    ...
    for a in urls:
        ...
        aozhou_single = res.xpath(f"//*[@class='metadataTable']/tbody/tr[1]/following-sibling::tr/td[8]//text()").getall()
        for b in aozhou_single:

            c = b.replace(",",'').strip()
            try:
                aozhou_all.append(int(c))
            except:
                aozhou_all.append(float(c))
